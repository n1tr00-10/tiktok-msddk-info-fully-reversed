...
## ⚖️ Differences: X-Mssdk-Info vs X-Gnarly / X-Dynosaur
...
Testing against TikTok live endpoints (`https://www.tiktok.com/node/report/reasons`):

```python
headers = {
    "User-Agent": ua,
    "Referer": "https://www.tiktok.com/",
    "X-Mssdk-Info": get_X_Mssdk_Info(user_agent=ua),
    "X-Gnarly": x_gnarly_signature,
    "Cookie": f"ttwid={ttwid}; msToken={msToken};"
}
response = requests.get(signed_url, headers=headers)
print(response.status_code)  # 200 OK
print(response.json()["status_code"])  # 0 (Success)
```
